...
import com.google.sps.data.comment_object;
...
public class DataServlet extends HttpServlet 
{
  ...
  public void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException 
  {
    ...
    Query query = new Query(Constants.COMMENT_OBJECT).addSort(Constants.TIMESTAMP, SortDirection.DESCENDING);
   
    PreparedQuery results = datastore.prepare(query);

    List<comment_object> comment_list= new ArrayList<>();
    for (Entity entity : results.asIterable()) {
      ...
      comment_object com_ = new comment_object(id, comment, timestamp);
    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response) throws IOException
{
    ...
    Entity commentEntity = new Entity(Constants.COMMENT_OBJECT);
   ...
   //sending response back to index(main) page
